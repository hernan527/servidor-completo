...
const atributtes_supabase_1 = require("../services/atributtes.supabase");
const getItems = async (req, res) => {
    // console.log('hola getItems clinicas')
    try {
        const response = await (0, atributtes_supabase_1.getProducts)();
...
const getItemById = async ({ params }, res) => {
    try {
        const { id } = params;
        const response = await (0, atributtes_supabase_1.getProduct)(id);
...
const createItem = async (req, res) => {
    try {
        const responseItem = await (0, atributtes_supabase_1.createProduct)(req);
...
const updateItem = async ({ params, body }, res) => {
    try {
        const { id } = params;
        const response = await (0, atributtes_supabase_1.updateProduct)(id, body);
        res.send(response);
    }
    catch (e) {
...
const deleteItem = async ({ params }, res) => {
    try {
        const { id } = params;
        const response = await (0, atributtes_supabase_1.deleteProduct)(id);
        res.send(response);
    }
    catch (e) {
...
const searchItem = async ({ params }, res) => {
    try {
        ...
        const response = await (0, atributtes_supabase_1.searchProducts)(query);
        res.send(response);
    }
    catch (e) {
